...
等待任务 → 开发中 → 等待审查 → 修复中 → 等待审查 → 归档中 → 已完成
...
**触发条件：** 检查 task-publish-repo 和 task.json

**执行逻辑：**
1. 拉取 task-publish-repo 最新代码
2. 读取 task-config.json
3. 拉取开发仓库最新代码
4. 读取 task.json 中的开发者状态、审查结果和里程碑状态
5. 根据状态决定是否需要执行操作
